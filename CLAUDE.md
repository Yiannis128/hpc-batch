...
- `jobs.py` — the `Job` dataclass shared by the queue, `state.json` and `info.json`. Converts itself to a `Request` and back to an `Allocation`. It also owns reading `state.json` (`read_state`), so the daemon and `--purge` tolerate the same set of broken files, and the pid pair: `attach_process()` writes `pid` and `proc_start` together, because the two questions asked of them later need both. Those are `identifiable()` ("was enough recorded to tell?") and `still_alive()`, which answers no unless it is. Callers SIGKILL a process group on the answer, so guessing is not on offer — but the two halves are not interchangeable, and asking only `still_alive()` silently merges "cannot say" into "gone". The daemon does that on purpose (`_probe` finalizes an unidentifiable job); `--purge` must not, because it reports what it could not kill.
...
- `client.py` — thin.
- `install.py` — picks the first of `util.ADMIN_GROUPS` that exists on the host and rewrites the systemd unit from the packaged template on *every* run, upgrades included. Reordering that tuple silently moves admin control over every job on a box that has both `wheel` and `sudo`. `install.json` in the prefix records what a run chose. `settle()` governs what `install()` does with it: an option not passed comes from the record rather than from the default, so `detect_admin_group()` cannot hand admin to a group that appeared on the box after the install, and an upgrade cannot relocate the entry points and orphan the old ones; an option passed with a different value is refused, because nothing here migrates an existing install. `uninstall()` deliberately does not settle — `bin_dirs_to_clean()` unions the recorded and requested directories, since cleanup wants every place a symlink might be. `--purge` additionally deletes what the *daemon* owns, and `install.json` cannot say where that is: the installer never chose it. Those paths come from `daemon_exec_start()`, read before the unit is removed and via `systemctl show` so a drop-in counts. `KillMode=process` leaves jobs running with nothing left to reap them, so purging kills them two ways, because neither alone covers the machine: `kill_running_jobs()` SIGKILLs the pids `state.json` recorded (the only handle that exists under `--no-cgroups`), and `CgroupManager.destroy()` takes the tree down (which catches anything a job forked away from its own process group). Both run before the state directory goes, since deleting it destroys the evidence. `purge()` returns whether everything it names actually went, and the closing line and the exit status both hang off that: a job it could not kill leaves nothing on disk naming it, and `uninstall.sh` execs the installer, so exit 2 is what a scripted teardown sees. Its `warn()` is the only way to report a problem, on the same principle as `_refuse()` in `cgroup.py`: printing a warning and marking the purge incomplete cannot come apart, so a step added later cannot warn and still report a clean machine. Tests point it at a scratch tree by patching `cgroup.CGROUP_FS`, as `test_cgroup.py` does, rather than by `purge()` taking a manager it would otherwise never vary. The installer must stay importable when the daemon's own modules are not — it is what you reach for when an install is broken — so it reaches for `cgroup.py`, `jobs.py`, `protocol.py` and `util.py`, never `daemon.py`.
