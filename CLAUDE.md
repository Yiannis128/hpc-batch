...
**The daemon owns `CUDA_VISIBLE_DEVICES`.** `_job_env` composes `defaults | job.env | ours` so a forwarded `--env` can never win. `job.env` is one layer, not two: the client composes it as `--env sweep | leading NAME=VALUE words`, so the full chain is `defaults | (sweep | assignments) | ours` and only the last term is the daemon's. It is rendered by `format_id_list`, which also writes `cpuset.cpus` and `cpuset.mems` — cpuset accepts ranges (`0-3`) and `CUDA_VISIBLE_DEVICES` does not, so compressing them for the cgroup's benefit would quietly hide GPUs from every job.
